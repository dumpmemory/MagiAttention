 ...
 public:
  ...
  void operator()(Params const& params, char* smem_buf) {
    ...
    pipeline_params.role =
        warp_group_idx == 0 ? MainloopPipeline::ThreadCategory::Producer : MainloopPipeline::ThreadCategory::Consumer;
    ...
    PipelineParams_dO pipeline_params_dO{
        pipeline_params.transaction_bytes, role_dO, pipeline_params.is_leader, pipeline_params.num_consumers};
    ...
    if (warp_group_idx == 0) { // Producer
        ...
        for (auto work_tile_info = scheduler.template get_initial_work</*IsProducerWarp=*/true>(params.scheduler);
          ...
          if constexpr (RangeMerge) {
            ...
            for (int idx = 0; idx < loop_count; ++idx) {
              ...
              bool tile_valid_tmp = mainloop.load(params.mainloop,
                                                  pipeline_q,
                                                  pipeline_do,
                                                  smem_pipe_write,
                                                  smem_pipe_write_do,
            ...
            tile_valid = mainloop.load(params.mainloop,
                                       pipeline_q,
                                       pipeline_do,
                                       smem_pipe_write,
                                       smem_pipe_write_do,
      ...
      for (auto work_tile_info = scheduler.template get_initial_work</*IsProducerWarp=*/false>(params.scheduler);
        ...
        Tensor tdKrdK =
            partition_fragment_C(tiled_mma_dKV, select<!dKV_swapAB ? 1 : 2, !dKV_swapAB ? 2 : 1>(TileShape_MNK{}));
        Tensor tdVrdV =
            partition_fragment_C(tiled_mma_dKV, select<!dKV_swapAB ? 1 : 2, !dKV_swapAB ? 2 : 1>(TileShape_MNK{}));
        clear(tdKrdK);
        clear(tdVrdV);
        ...
        if constexpr (RangeMerge) {
          ...
          for (int idx = 0; idx < loop_count; ++idx) {
            ...
            bool tile_valid_tmp = mainloop.mma(params.mainloop,
                                               pipeline_q,
                                               pipeline_do,
                                               smem_pipe_read,
                                               smem_pipe_read_do,
          ...
          tile_valid = mainloop.mma(params.mainloop,
                                    pipeline_q,
                                    pipeline_do,
                                    smem_pipe_read,
                                    smem_pipe_read_do,
