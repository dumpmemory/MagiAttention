...
class BinaryGreedyParallelDynamicAttnAlgorithm(DynamicAttnAlgorithm):
    """The Binary-Greedy-Parallel dynamic dispatch algorithm implementation (Parallel version of BinaryGreedy)"""

    def __init__(self, debug_print: bool = False):
        """
        The init method of the Binary-Greedy-Parallel dynamic dispatch algorithm
        """
        self.debug_print = debug_print

    @property
    def type(self) -> DynamicAttnAlgType:
        return DynamicAttnAlgType.BINARY_GREEDY_PARALLEL
    ...
    def _GreedySelection(
        ...
        # We use wj / max(cj, 1e-6) as a simple heuristic
        indexed_edges = []
    ...
    def _GreedyMaxFlow(
        ...
        # Return format: (is_feasible, assignment result)
        return is_feasible, sparse_res
    ...
    def binary_greedy(
        ...
        """
        Core Binary-Greedy solver.

        Args:
            cp_size: Number of ranks.
            rank_m: Host rank index for each Q interval.
            rank_n: Host rank index for each K interval.
            comm_len_m: Sequence length for each Q interval.
            comm_len_n: Sequence length for each K interval.
            sparse_area_map: List of (i, j, area) for each sparse grid cell.
            num_heads_q: Number of Q heads.
            num_heads_kv: Number of KV heads.
            usp_choices: Heuristic preferred rank for each Q interval.
            rank: Current process rank (for logging / timing).
        ...
        for iter_idx in range(max_iters):
            ...
            for _attempt in range(max_attempts):
                ...
                success, solver_try = self._GreedyMaxFlow(
                    cp_size,
                    edges,
                    selected_edges,
                    sparse_area_map,
                ...
                if rank == 0 and self.debug_print:
                    print(f"    - Iter {total_iters}: mid={mid:.2f}, success={success}")
        ...
        if not success:
            if rank == 0 and self.debug_print:
                print(
                    "[BinaryGreedyParallelDynamicAttnAlgorithm] network flow failed, "
    ...
    def solve(
        ...
        """
        The solve method of the Binary-Greedy-Parallel dynamic dispatch algorithm
