...
class PackGQA:
    ...
    def compute_ptr(
        ...
        for i in cutlass.range_constexpr(num_ptr_per_thread):
            ...
            tPrPtr[i] = cutedsl_utils.elem_pointer(tensor, ((h_idx, m_idx),)).toint()
        return tPrPtr
    ...
    def load_Q(
        ...
        tQpQ = cutedsl_utils.predicate_k(tQcQ, limit=mQ.shape[1])
        ...
        for m in cutlass.range_constexpr(cute.size(tQsQ.shape[1])):
            q_ptr_i64 = cutedsl_utils.shuffle_sync(
    ...
    def store_LSE(
        ...
        for m in cutlass.range_constexpr(cute.size(tLSErLSE)):
            lse_ptr_i64 = cutedsl_utils.shuffle_sync(
    ...
    def store_O(
        ...
        for m in cutlass.range_constexpr(cute.size(tOrO.shape[1])):
            o_ptr_i64 = cutedsl_utils.shuffle_sync(
