...
class Softmax(ParamsBase):
    ...
    def _compute_row_max(
        ...
        return cutedsl_utils.fmax_reduce(acc_S_row, init_val, arch=self.arch)

    def _compute_row_sum(
        self, acc_S_row_exp: cute.TensorSSA, init_val: float | Float32 | None = None
    ) -> Float32:
        return cutedsl_utils.fadd_reduce(acc_S_row_exp, init_val, arch=self.arch)
    ...
    def online_softmax(
        ...
        for r in cutlass.range(cute.size(row_max), unroll_full=True):
            ...
            row_max_cur = cutedsl_utils.fmax_reduce(
                acc_S_row,
            ...
            if cutlass.const_expr(is_first):
                ...
                acc_S_row_sum = cutedsl_utils.fadd_reduce(
    ...
    def finalize(
        ...
        row_sum.store(cutedsl_utils.warp_reduce(row_sum.load(), operator.add, width=4))
...
def apply_score_mod_inner(
    ...
    batch_idx_ssa = cutedsl_utils.scalar_to_ssa(batch_idx, cutlass.Int32).broadcast_to(
        (vec_size,)
    )
    ...
    for i in cutlass.range(0, n_vals, vec_size, unroll_full=True):
        for j in cutlass.range(vec_size, unroll_full=True):
            ...
            if cutlass.const_expr(qhead_per_kvhead > 1 and constant_q_idx is None):
                ...
                # Building up the logical q_head idx: final_q_head = kv_head * qhead_per_kvhead + (q_physical % qhead_per_kvhead)  # noqa: E501
