...
class PagedKVManager(ParamsBase):
    ...
    def create(
        ...
        tKpK = cutedsl_utils.predicate_k(tKcK, limit=mK_paged.shape[1])
        ...
        if const_expr(head_dim_padded == head_dim_v_padded):
            ...
            tVpV = cutedsl_utils.predicate_k(
    ...
    def compute_X_ptr(self, K_or_V: str):
        ...
        for i in cutlass.range(self.page_entry_per_thread, unroll=1):
            ...
            if const_expr(transposed):
                tPrXPtr[i] = cutedsl_utils.elem_pointer(
                    mX, (0, page_offset, page)
                ).toint()
            else:
                tPrXPtr[i] = cutedsl_utils.elem_pointer(
                    mX, (page_offset, 0, page)
                ).toint()
        return tPrXPtr
