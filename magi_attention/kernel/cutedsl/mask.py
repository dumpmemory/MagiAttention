        ...
        if const_expr(not mask_causal and not mask_local and mask_mod is None):
            ...
            for r in cutlass.range_constexpr(nrow):
                ...
                for col in cutlass.range_constexpr(ncol):
                    ...
                    batch_idx_ssa = cutedsl_utils.scalar_to_ssa(
                        batch_idx, cutlass.Int32
                    )
                    head_idx_ssa = cutedsl_utils.scalar_to_ssa(
                        head_idx_for_mod, cutlass.Int32
                    )
                    q_idx_ssa = cutedsl_utils.scalar_to_ssa(row_for_mod, cutlass.Int32)
                    kv_idx_ssa = cutedsl_utils.scalar_to_ssa(col_for_mod, cutlass.Int32)
                    mask_value = mask_mod(
                        batch_idx_ssa,
                        head_idx_ssa,
                        q_idx_ssa,
    ...
    def apply_mask_sm100(
        ...
        if const_expr(rBitmask is not None):
            ...
            for i in cutlass.range_constexpr(ncol):
                ...
                mask_row_ssa = cutedsl_utils.scalar_to_ssa(
                    mask_row_for_mod, cutlass.Int32
                )
                kv_idx_ssa = cutedsl_utils.scalar_to_ssa(
                    global_col_for_mod, cutlass.Int32
                )
                mask_value = mask_mod(
                    batch_idx_ssa,
                    head_idx_ssa,
            ...
            if const_expr(mask_causal):
                ...
                #     cute.printf("tidx = %d, tidx tmem = %d, row_idx = %d, col_limit_right = %d, causal_row_offset = %d\n", cute.arch.thread_idx()[0], thr_tmem_load.thr_idx, row_idx, col_limit_right, causal_row_offset)  # noqa: E501
                ncol = const_expr(cute.size(tScS_t2r.shape))
                if const_expr(not r2p):
                ...
                if const_expr(not r2p):
                    # if cute.arch.thread_idx()[0] == 0 or cute.arch.thread_idx()[0] == 128: cute.printf("m_block = {}, n_block = {}, row_idx = {}, causal_row_offset = {}, col_limit_right = {}, col_limit_left = {}", m_block, n_block, row_idx, causal_row_offset, col_limit_right, col_limit_left)  # noqa: E501
                    for i in cutlass.range(cute.size(tScS_t2r.shape), unroll_full=True):
    ...
    def apply_mask_sm100_transposed(
        ...
        if const_expr(not mask_causal and not mask_local and mask_mod is not None):
            ...
            if is_full_block:
                ...
                batch_idx_ssa = cutedsl_utils.scalar_to_ssa(batch_idx, cutlass.Int32)
                head_idx_ssa = cutedsl_utils.scalar_to_ssa(head_idx, cutlass.Int32)
                ...
                for i in cutlass.range_constexpr(ncol):
                    ...
                    q_idx_ssa = cutedsl_utils.scalar_to_ssa(
                        q_idx_for_mod, cutlass.Int32
                    )
                    kv_idx_ssa = cutedsl_utils.scalar_to_ssa(
                        kv_idx_for_mod, cutlass.Int32
                    )
            ...
            if const_expr(mask_causal):
                ...
                # if tidx < 32:
                #     cute.printf("tidx = {}, {} {}, {} {}", tidx, tScS_t2r[0][0], tScS_t2r[0][1], tScS_t2r[1][0], tScS_t2r[1][1])  # noqa: E501
...
class Sm100MaskEnum(enum.Enum):
    ...
    - WINDOW_MASK_BWD_INFERENCE: Same as the window mask for backward pass, but has
      the limitation that the end of q is aligned with the end of k
    """
